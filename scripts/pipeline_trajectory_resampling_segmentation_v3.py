...
query_dynamic_ships_resampled_segmented = "INSERT INTO ais_data.dynamic_ships_min_trip_card_3_segmented_12h_resampled_1min (mmsi, turn, speed, course, heading, lon, lat, ts, datetime, traj_id, traj_id_temporal_gap, real_point, velocity) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);"
...
for i in tqdm(range(0, len(mmsi_list), mmsi_list_window_size)):
    ...
    # gsplt.map_plot(traj, column='ts', figsize=(10,15), title="A Vessel's 'raw' trajectory", fontsize=10, color=[None], legend=True)  
    ...
    for _, vessel in traj.groupby('mmsi'):
        vessel = gspp.segment_resample_v2(vessel, ports, port_epsg=2154, port_radius=2000, temporal_threshold=12, cardinality_threshold=3, rule = '60S', method='linear', crs = {'init': 'epsg:4326'}, drop_lon_lat = False)
        ...
        if os.path.exists(f'./test_data/nari_dynamic_min_card_3_resampled_and_segmented_{CLUSTER_ID}.csv'):
            with open(f'./test_data/nari_dynamic_min_card_3_resampled_and_segmented_{CLUSTER_ID}.csv', 'a') as f:
                vessel.to_csv(f, header=False, index=False)
        else:
            print ('...creating file...\n')
            with open(f'./test_data/nari_dynamic_min_card_3_resampled_and_segmented_{CLUSTER_ID}.csv', 'w') as f:
