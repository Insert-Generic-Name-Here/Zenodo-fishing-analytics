...
def calculate_acceleration(gdf, smoothing=True, window=3, center=False):
	'''
	...
	gdf['acceleration'] = gdf['acceleration'].rolling(window, center=center).mean().bfill().ffill()

	if smoothing:
		gdf.loc[:,'acceleration'] = gdf['acceleration'].rolling(window, center=center).mean().bfill().ffill()
...
def calculate_bearing(gdf, smoothing=True, window=3, center=False):
	'''
	...
	if smoothing:
		gdf.loc[:,'bearing'] = gdf['bearing'].rolling(window, center=center).mean().bfill().ffill()
...
for i in tqdm(range(0, len(mmsi_list), mmsi_list_window_size)):
	...
	traj = traj.groupby('mmsi', group_keys=False).apply(gspp.calculate_velocity, smoothing=True, window=3, center=False)
	print ('Stage 3: Calculating Acceleration')
	traj = traj.groupby('mmsi', group_keys=False).apply(calculate_acceleration, smoothing=True, window=3, center=False)
	print ('Stage 4: Calculating Bearing')
	traj = traj.groupby('mmsi', group_keys=False).apply(calculate_bearing, smoothing=False, window=3, center=False)
	
	print ('Checkpoint 1: Saving Calculated Trajectories...')
	if os.path.exists(f'./test_data/nari_dynamic_vanilla_features_{CLUSTER_ID}.csv'):
		with open(f'./test_data/nari_dynamic_vanilla_features_{CLUSTER_ID}.csv', 'a') as f:
			traj.to_csv(f, header=False, index=False)
	else:
		print ('...creating file...\n')
		with open(f'./test_data/nari_dynamic_vanilla_features_{CLUSTER_ID}.csv', 'w') as f:
