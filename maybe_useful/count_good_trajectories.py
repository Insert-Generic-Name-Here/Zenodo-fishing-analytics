...
CLUSTER_ID = 0 # 0 = MASTER; 1-5 = SLAVES
...
def vessel_starts_and_docks_in_port(ves_seg, ports):
    ...
    points_outside_geometry = ves_seg[~ves_seg.isin(points_within_geometry)].dropna()
...
mmsi_list = pd.read_pickle(os.path.join('.', 'data', 'pkl', 'mmsi_list.pckl'))[0]
PORT_RADIUS = 2000 # Port Radius in Km
...
mmsi_traj_report = pd.DataFrame(columns=["mmsi", "nr_of_AIS_signals", "nr_of_segments_type1", "nr_of_segments_type2"])
...
for i in range(0, len(mmsi_list), mmsi_list_window_size):
    ...
    for idx, mmsi in tqdm(enumerate(mmsis), total=mmsi_list_window_size):
        ...
        mmsi_traj_report = mmsi_traj_report.append({"mmsi":mmsi, "nr_of_AIS_signals":len(vessel), "nr_of_segments_type1":len(break_points)+1, "nr_of_segments_type2":good_traj_no, "nr_of_AIS_signals_in_segments_type2":good_traj_ais_points}, ignore_index=True)
    break
    print ('\n')
    
print ('\nSaving Result Data to CSV...')
mmsi_traj_report.to_csv(f'./data/csv/mmsi_traj_report_{CLUSTER_ID}.csv', index=False, header=True)